...
while (( "$#" )); do
  case "$1" in
    ...
    --lig)
      ...
      if [ "$#" -lt 2 ]; then
        echo "Error: --lig requires a comma-separated list of CCD codes or SMILES files." >&2
        exit 1
      fi
      LIGANDS_STR="$2"
...
if [ ${#FASTA_FILES[@]} -eq 0 ]; then
  ...
  echo "  - To specify a SMILES ligand, use a .sml file (e.g., 'myligand.sml' or 'myligand.sml:2' for 2 copies)."
  echo ""
  echo "Example for a heterotrimer with two identical subunits and SMILES ligand:"
  echo "  $0 proteinA.fa proteinA.fa proteinB.fa --lig SAH:2,GTP,mycompound.sml"
  echo "  # This will create a job folder named '2xproteinA-proteinB-2xSAH-GTP-mycompound'"
  exit 1
fi
...
# Arrays to track ligand information
LIGAND_ENTRIES=()  # Stores complete ligand info (code/smiles, count, type)
declare -A LIGAND_TYPE  # Maps ligand identifier to type (ccd or smiles)
declare -A LIGAND_SMILES  # Maps ligand identifier to SMILES string
...
if [ -n "$LIGANDS_STR" ]; then
    ...
    for item in "${LIGAND_ARRAY[@]}"; do
        ...
        # Check if this is a SMILES file
        if [[ "$code" == *.sml ]]; then
            # This is a SMILES file
            smiles_filepath="$BASE_INPUT_DIR/$code"
            if [ ! -f "$smiles_filepath" ]; then
                echo "Error: SMILES file not found: $smiles_filepath" >&2
                exit 1
            fi
            
            # Read SMILES string from file
            smiles_string=$(cat "$smiles_filepath")
            # Remove any trailing whitespace/newlines
            smiles_string=$(echo -n "$smiles_string")
            
            # Use the filename without extension as identifier
            ligand_id="${code%.sml}"
            
            # Store the ligand information
            for ((j=0; j<count; j++)); do
                LIGAND_ENTRIES+=("$ligand_id|smiles")
            done
            LIGAND_TYPE["$ligand_id"]="smiles"
            LIGAND_SMILES["$ligand_id"]="$smiles_string"
        else
            # This is a CCD code
            for ((j=0; j<count; j++)); do
                LIGAND_ENTRIES+=("$code|ccd")
            done
            LIGAND_TYPE["$code"]="ccd"
        fi
    done
fi

# Build Stoichiometric Ligand Name Part
if [ ${#LIGAND_ENTRIES[@]} -gt 0 ]; then
  ...
  LIGAND_NAME_PARTS=()
  # Get unique ligands preserving order
  UNIQUE_LIGANDS=()
  for entry in "${LIGAND_ENTRIES[@]}"; do
    ligand_id="${entry%|*}"
...
for entry in "${LIGAND_ENTRIES[@]}"; do
  ...
  if [[ "$ligand_type" == "smiles" ]]; then
    # For SMILES ligands, use jq to properly escape the SMILES string
    smiles_string="${LIGAND_SMILES[$ligand_id]}"
    escaped_smiles=$(echo -n "$smiles_string" | jq -R .)
...
echo $FINAL_JOB_NAME >> /scratch/groups/ogozani/alphafold3/folding_jobs_nsd2i.csv
